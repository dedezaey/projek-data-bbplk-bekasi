...
if(isset($_POST['submitTemp'])){
  ...
  //Akan dijalankan jika tombol Simpan Transaksi dipilih 
	$noTransaksi = $_POST['noTransaksi']; $tglTransaksi = $_POST['tanggalTransaksi']; $idUser = $_POST['idUser']; $grandTotal = $_POST['grandTotal']; $sql1 = "INSERT INTO tb_header_transaksi VALUES('$noTransaksi','$tglTransaksi','$idUser','$grandTotal')"; if($conn->query($sql1) === TRUE){ 
  //Jika berhasil maka semua record dari tb_temp akan dipindahkan ke tb_detail_transaksi 

		$sql2 = "SELECT*FROM tb_temp"; 
		$result = $conn->query($sql2); while($row=$result->fetch_assoc()){ 
   ...
   //Mengosongkan tb_temp 
		...
		echo "<script>alert('Data Gagal Disimpan')</script>";
		echo "<script>window.location.assign('index.php?p=FormInputTransaksi.php')</script>"; }
	}else{echo "<script>window.location.assign('index.php?p=FormInputTransaksi.php')</script>"; }
