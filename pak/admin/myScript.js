...
//Memanggil fungsi showClock(); showClock();
//Fungsi untuk menampilkan jam dengan detik yang dinamis function showClock() {
	var x = document.getElementById('clock'); t = setInterval(clock, 1000);
	function clock() {
		var d = new Date();
		x.innerHTML = addZero(d.getHours()) + ":" + addZero(d.getMinutes()) + ":" + addZero(d.getSeconds());
	}
}
//Fungsi untuk menambahkan 0 di depan jam/menit/detik ketika terdiri 1 digit function addZero(a) {
	if (a < 10) {
		a = "0" + a;
	}
	return a
...
//Fungsi untuk menampilkan halaman form update user function showUserUpdateForm(id, nama, email) {
//document.getElementById adalah cara pada js DOM untuk mengambil elemen pada halaman html document.getElementById('modal-id-user').value = id;
document.getElementById('modal-nama-user').value = nama; 
document.getElementById('modal-email-user').value = email;
}
//Fungsi untuk menampilkan halaman form update user function showBarangUpdateForm(id, nama, satuan, harga) {
//document.getElementById adalah cara pada js DOM untuk mengambil elemen pada halaman html document.getElementById('modal-id-barang').value = id;
...
$.ajax({
	url: "getAjaxDataBarang.php", type: 'POST',
	dataType: 'json', data: {
		id: id,
	},
	success: function (result) {
		$.each(result, function (index, el) { if (index == "harga_barang") {
			harga = el
		} else if (index == "satuan_barang") { satuan = el
		}
	});
...
//dan grandTotal lebih dari 0; function validasiTransaksi() {
